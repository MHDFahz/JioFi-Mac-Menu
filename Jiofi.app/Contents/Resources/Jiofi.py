...
if __name__ == "__main__":
    def printit(val, lav):
        ...
        if a == 100 and (temp == 1 or lav == 2 or lav ==7):
            # print(temp)
            # print("hello")
            rumps.notification(name.get_text(), status, "JioFi is Fully Charged")
            temp = temp + 1
            lav = 5
        if a <= 20 and (lab==2 or lav==3 or lav==5):
            # print("working")
            rumps.notification(name.get_text(), "Battery Low", "JioFi is Fully Charged")
            lav = 7
            
        
        threading.Timer(60, printit,[3,lav]).start()
